...
def train(epoch, loader, wav2vec2_processor, wav2vec2_model, dsnet, optimizerB, scheduler, device):
    ...
    for i, (img_list, audio_path) in enumerate(loader):
        ...
        loss1 = criterion2(ds_result[:,96:136,:], all_preds[:,96:136,:])
        loss2 = criterion(ds_result, all_preds)
        loss = loss1 + loss2
        wav2vec_loss += loss.item()
        ...
        if dist.is_primary():
            ...
            if i % 100 == 0:
                ...
                waveform = audio.load_wav(audio_path[0], sr=16000) 
                input_value = wav2vec2_processor(waveform, sampling_rate=16000, return_tensors='pt').input_values
                input_value = input_value.to(device)
...
def main(args):
    ...
    for i in range(args.epoch):
        SAVE_PATH = f"checkpoint/audio2landmark_masked_{str(i + 1).zfill(3)}.pt"
