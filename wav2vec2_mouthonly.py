...
class DSNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_seq = nn.Conv1d(40, 512, kernel_size=3, stride=1, padding=1)
        ...
        self.linear4 = nn.Conv1d(128, 40, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, len_img, refer, x):
        try:
            ...
            all_seq = torch.zeros((len_img, 40, 1)).to(device)
            for c, s in enumerate(splits):
                if s.size(1) == 2 and c < len_img:
                    s = self.linear1(s)
                    s = self.ReLU(s)
                    s_t = s.permute(0, 2, 1)
...
def train(epoch, loader, wav2vec2_processor, wav2vec2_model, dsnet, optimizerB, scheduler, device):
    ...
    for i, (img_list, audio_path) in enumerate(loader):
        ...
        all_preds = torch.zeros((len_img, 40, 1)).to(device)    # [48:68, :]
        refer = []
        for t, fname in enumerate(img_list):
            ...
            try:
                ...
                preds = preds / 160
                preds_mouth = preds[48:68, :]
                preds_wo_mouth = preds[:48, :]
                ...
                # cnt += 1
                preds_tensor = torch.tensor(preds_mouth).to(device)
                ow_tensor = torch.tensor(preds_wo_mouth).to(device)
                y = preds_tensor.view(1, 40)
        ...
        if dist.is_primary():
            ...
            if i % 100 == 0:
                ...
                len_img = len(img_list)
                all_preds = torch.zeros((len_img, 40, 1))
                all_ow = torch.zeros((len_img, 96, 1))
                refer = []
                for t, fname in enumerate(img_list):
                    # optimizer.zero_grad()
                    # image = Image.open(fname[0]).convert('RGB')
                    try:
                        ...
                        preds_tensor = torch.tensor(preds_mouth).to(device)
                        ow_tensor = torch.tensor(preds_wo_mouth).to(device)
                        y_ow = ow_tensor.reshape(1, 96)
                        y = preds_tensor.reshape(1, 40)
                        y = y.unsqueeze(-1).to(device)
                        y_ow = y_ow.unsqueeze(-1).to(device)
                        all_preds[t] = y
                        all_ow[t] = y_ow
                        if t == 0:
                            refer = y
                ...
                with torch.no_grad():
                    ...
                    out = dsnet(len_img, refer, audio_output)
                    out = out.to(device)
                    all_ow = all_ow.to(device)
                    out = torch.cat((all_ow, out), dim=1)
                    # print(out.shape)
                    ...
                    # save_img.save('result4/result_{}.png'.format(i), 'PNG')
                    ...
                    #     plt.close(fig)
                    # save_img= Image.fromarray(result_list)
                    # save_img.save('result4/preds_{}.png'.format(i), 'PNG')
...
def main(args):
    ...
    for i in range(args.epoch):
        SAVE_PATH = f"checkpoint/audio2landmark_dsnet_mouth_{str(i + 1).zfill(3)}.pt"
